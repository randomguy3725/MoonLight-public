        ...
        if (customRotationSetting.get()) {
            switch (calcRotSpeedMode.get()) {
                case "Linear":
                    ...
                    break;
                case "Acceleration":
                    RotationUtils.setRotation(rots,moveFix.get() ? moveFixMode.is("Strict") ? MovementCorrection.STRICT : MovementCorrection.SILENT : MovementCorrection.OFF, maxAcceleration.get(), accelerationError.get(), constantError.get(), smoothlyResetRotation.get());
                    break;
            }
        } else {
